...
export function previewCalendar(host,api,onSelect){
 ...
 async function render(){
  ...
  try{
   ...
   host.innerHTML=`<div class="calendar-toolbar"><button type="button" id="month-prev" aria-label="Previous month">←</button><h2>${esc(heading)}</h2><button type="button" id="month-next" aria-label="Next month">→</button><label>Jump to month<input id="calendar-month" type="month" min="2000-01" max="2099-12" value="${month}"></label><button type="button" id="calendar-today">Today</button></div><p class="muted">Select a day to see Shul View. Hebrew dates below are for daytime; the evening preview uses the Hebrew date after sunset.</p><div class="month-grid">${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=>`<div class="calendar-weekday">${d}</div>`).join('')}${'<span class="calendar-empty"></span>'.repeat(data.offset)}${data.days.map(d=>`<button type="button" class="calendar-day" data-date="${d.date}" aria-pressed="${d.date===selected}" ${d.date===localStamp().slice(0,10)?'aria-current="date"':''} aria-label="${esc(d.date+' '+d.hebrew+' '+d.holidays.join(', '))}"><strong>${d.day}</strong><span class="hebrew-date" dir="rtl">${esc(d.hebrew)}</span>${d.holidays.map(h=>`<span class="calendar-holiday" dir="rtl">${esc(h)}</span>`).join('')}</button>`).join('')}</div>`;
