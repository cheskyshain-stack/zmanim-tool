...
async function edit(s, unsaved = false) {
  ...
  const update = async () => {
    ...
    if (kind !== "schedule") {
      ...
      document.querySelector("#length-warning").textContent = (v.data.message || "").length > 220 || (v.data.dedicationText || "").length > 180 || (v.title || "").length > 70 || (v.data.dedicationName || "").length > 70 || (v.data.sponsor || "").length > 70 ? "This is long for a big screen. Shorten it, or preview the continuation cards before publishing." : "";
    } else {
