...
async function edit(s, unsaved = false) {
  ...
  let fields = "";
  if (kind === "announcement") fields = `${select("Start with a template", "template", [["", "Choose a template"], ...categories], d.category || "")}${input("Internal name (private)", "internalName", s.internalName)}${input("Visible title", "title", s.title, "text", 'maxlength="200" dir="auto"')}${select("Category", "category", categories, d.category || categories[6])}${area("Message", "message", d.message)}${input("Contact name (optional)", "contact", d.contact, "text", 'dir="auto"')}${input("Phone number (optional)", "phone", d.phone, "tel", 'dir="ltr"')}${select("Placement", "placement", [["automatic", "Automatic"], ["left", "Left"], ["right", "Right"]], d.placement || "automatic")}${select("Priority", "priority", [["normal", "Normal"], ["important", "Important"], ["urgent", "Urgent"]], d.priority || "normal")}${select("Display behavior", "behavior", [["rotating", "Rotating"], ["pinned", "Pinned"]], d.behavior || "rotating")}${input("Seconds per card (15–120)", "duration", d.duration || 25, "number", 'min="15" max="120"')}`;
  ...
  const update = async () => {
    ...
    if (kind !== "schedule") {
      document.querySelector("#card-preview").innerHTML = cardHTML(announcementPages({ ...v, title: v.title || "Your title", data: v.data })[0]);
      document.querySelector("#length-warning").textContent = (v.data.message || "").length > 220 || (v.data.dedicationText || "").length > 180 || (v.title || "").length > 70 || (v.data.dedicationName || "").length > 70 || (v.data.sponsor || "").length > 70 ? "This is long for a TV. Shorten it, or preview the continuation cards before publishing." : "";
    } else {
      document.querySelector("#card-preview").innerHTML = '<div class="notice">Use Preview to see this schedule with the actual TV layout and next minyan.</div>';
  ...
  async function save(status) {
    ...
    try {
      ...
      if (status === "published") {
        ...
        if (!await confirm("Publish to the TV display?", document.querySelector("#exact-times").textContent + (check2.warnings.length ? " Review: " + check2.warnings.join(" ") : ""), "Publish")) return;
      }
