...
try {
 ...
 for(const date of ['2026-09-24','2026-10-15']){
  ...
  for(const theme of ['light','dark']){
   ...
   const result=await page.evaluate(snapshot=>{
    ...
    const overflow=[];
    for(const e of root.querySelectorAll('.board-week-body,.board-shabbos-body,.board-notices,.board-community,.board-zmanim,.sheet-column,.sheet-row,.board-notices .tv-card,.board-community .tv-card,.board-dedication .tv-card')){
    ...
    const center=root.querySelector('.tv-center'),sheet=root.querySelector('.schedule-sheet-box'),header=sheet?.querySelector('.sheet-header');
    return {theme:root.dataset.theme,overflow,notices:root.querySelectorAll('.board-notices>.tv-card').length,noticeTitles:[...root.querySelectorAll('.board-notices .tv-card h2')].map(e=>e.textContent),zmanim:root.querySelectorAll('.board-zmanim>div').length,dedication:root.querySelector('.board-dedication')?.textContent,sheetPresent:!!sheet,sheetContained:!sheet||(!outside(sheet,center)&&[...sheet.querySelectorAll('.sheet-column')].every(c=>!outside(c,center))),sheetColumns:sheet?.querySelectorAll('.sheet-column').length,sheetFooter:!!sheet?.querySelector('footer,.sheet-footer'),sheetHeaderChildren:header?[...header.children].map(e=>e.tagName):[],sheetTitle:header?.textContent,sheetIds:[...root.querySelectorAll('[data-sheet-source]')].map(e=>e.dataset.sheetSource),sourceIds:[...root.querySelectorAll('[data-source-id]')].map(e=>e.dataset.sourceId),weeklyServices:[...root.querySelectorAll('.board-service>h3')].map(e=>e.textContent)};
   },snapshot);
   ...
   if(schedule.fullSheet){
    check(result.sheetPresent&&result.sheetContained,`${name}: sheet stays inside schedule area`,result.sheetContained);
    check(result.sheetColumns===3,`${name}: exactly three sheet columns`,result.sheetColumns);
    check(!result.sheetFooter,`${name}: no sheet footer`);
    check(JSON.stringify(result.sheetHeaderChildren)==='["H1"]',`${name}: header contains title only`,result.sheetHeaderChildren);
    check(result.sheetTitle===`${schedule.fullSheet.title} · ${schedule.fullSheet.yearLabel||schedule.fullSheet.year}`,`${name}: title is Yom Tov and year only`,result.sheetTitle);
    check(sameIds(result.sheetIds,schedule.fullSheet.blocks.flatMap(b=>b.rows.map(r=>r.id))),`${name}: every special-sheet source row appears exactly once`,result.sheetIds);
   }else{
...
if(issues.length){console.error(JSON.stringify(issues,null,2));process.exitCode=1;}else console.log('Board layout passed: special and ordinary schedules, Light/Dark, complete sources, actual card bounds, theme/rotation continuity.');
