...
def stands_flush(blocks) -> bool:
    """Whether a card should stand flush instead of going onto a colon axis.

    The axis exists so a column of times does not wobble, and with several long hours in a
    block it earns its keep. With exactly one it does the opposite: the other lines get
    pulled in by a digit and the lone 12:30 reads as though it is sticking out to the left.

    So: one long hour on the whole card, alone on its line, with the other lines of its own
    block carrying two times each. That last part separates the שבת card, where a 12:30
    sits above three pairs, from a חול card, whose lines carry one time each and whose
    מעריב runs 10:00, 10:30 and 11:00 together, where the axis is still right.

    `blocks` is one list of plain text lines per row of the card.
    """
    lines = [(index, line.strip()) for index, block in enumerate(blocks) for line in block if line.strip()]
    long = [i for i, (_b, line) in enumerate(lines) if re.match(r"\d{2}:\d{2}", line)]
    if len(long) != 1:
        return False
    at = long[0]
    block_index, only = lines[at]
    if len(TIME.findall(only)) != 1:
        return False
    rest = [line for i, (b, line) in enumerate(lines) if b == block_index and i != at]
    return bool(rest) and all(len(TIME.findall(line)) == 2 for line in rest)
