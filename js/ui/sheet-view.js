...
export function renderSheet(container, state, sheet, onChange) {
  ...
  const columns = isKayitz ? KAYITZ_COLUMNS : CHOREF_COLUMNS;
  const buildRow = isKayitz ? buildKayitzRow : buildChorefRow;
  const weeks = sheet.weeks.map((w) => ({ ...w, date: new Date(w.date) }));
  const pages = splitWeeksIntoPages(weeks, sheet.pageSizes);
  const isEnglish = state.settings.language === 'en';
  const colOrder = isEnglish ? [...columns.map((c) => c.key), 'parsha'] : ['parsha', ...rtlOrdered(columns).map((c) => c.key)];
  const colLabel = { parsha: isEnglish ? 'Parsha' : 'פרשה', ...Object.fromEntries(columns.map((c) => [c.key, c.header.replace(/\n/g, ' ')])) };
  ...
  pages.forEach((pageWeeks, pageIndex) => {
    pagesEl.appendChild(renderPage(pageWeeks, pageIndex, totalPages, columns, buildRow, settings, sheet, state, onChange, isKayitz));
  });
...
function renderPage(pageWeeks, pageIndex, totalPages, columns, buildRow, settings, sheet, state, onChange, isKayitz) {
  ...
  const rows = pageWeeks
    .map((week) => {
      ...
      const ruled = applyRules(computed, week, state.rules, sheet.season, appliedColumns);
  ...
  page.querySelectorAll('.cell').forEach((cellEl) => {
    ...
    cellEl.addEventListener('blur', () => {
      ...
      const computed = splitBuild(sheet.season)({ ...week, date: new Date(week.date) }, settingsResolved);
      const ruled = applyRules(computed, { ...week, date: new Date(week.date) }, state.rules, sheet.season);
